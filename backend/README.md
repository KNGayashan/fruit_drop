...
- **Brands tab** — add/edit/deactivate/delete brands; each has a logo (shown on the brand-select screen), a score icon (shown during gameplay), an optional background image override, and primary/secondary theme colors. Selecting a brand re-themes the game's frame, HUD, badge, and penalty splat to that brand's colors in real time — this is how a single game can visually "become" whichever sponsor is currently playing.
